...
import { AllegatoCorrispondenza, DettaglioCorrispondenzaDTO, TemplateComunicazione } from '../../../features/gestione-istanza-comunicazioni/model/corrispondenza.models';
...
export class NuovaComunicazioneComponent implements OnInit {
  ...
  @Input("isBozzaIntegrazione")isBozzaIntegrazione: boolean=false;
  @Input("canEditRecipients") canEditRecipients: boolean = true;
  ...
  ngOnInit() {
    ...
    this.buildForm();
    if(this.isBozzaIntegrazione && 
      this.templateComunicazione && 
      this.selectOptionsTemplate.length==1 && 
      !this.toEdit.corrispondenza.codiceTemplate
      ){
        //preimposto il template....
        this.comForm.get('codiceTemplate').patchValue(this.selectOptionsTemplate[0].value);
        //simulo l'onchange
        this.cambioTemplate({value:this.selectOptionsTemplate[0].value});
        this.comForm.get('codiceTemplate').disable();
    }
    this.loaded = true;
  ...
  //get formArray(): FormArray { return <FormArray>this.comForm.controls.destinatari }
  ...
  public destinatariValidi(){
    if(this.isBozzaIntegrazione){
      //non sono obbligatori... verràaggiunto almeno il richiente in BE
      return true;
    }else{
      return this.destinatari && this.destinatari.length > 0;
    }
  }
  ...
  public sendData(bozza: boolean, conProtocollo: boolean): void {
    if (!bozza) {
      ...
      if (!this.comForm.valid || !this.destinatariValidi()) {
  ...
  public getPrefixOggetto(){
    ...
    if(!codicetemplate && this.codiceFascicolo){
      return "Fascicolo " +this.codiceFascicolo; 
    }else{
      return ""; 
    }
