...
import { GruppiRuoliDTO, LoggedUser, PlainTypeStringId } from '../model/logged-user';
...
import { fromGroupToGroupType } from 'src/app/core/functions/generic.utils';
...
export class RichiestaAbilitazioneComponent implements OnInit,OnDestroy {
  ...
  ngOnInit() {
    ...
    combineLatest([this.gestioneGruppoService.getGruppiOrganizzazioniConCL(),
                  this.gestioneGruppoService.getTestoPrivacy(),
                  this.gestioneGruppoService.readGruppi(),
                  this.gestioneGruppoService.getLastRichiestaAbilitazioneData()])
    ...
    .subscribe(
        ...
        if(this.myGruppi && this.myGruppi.length>0){
          this.myGruppi.forEach(tipoOrganizzazione=>{
            tipoOrganizzazione.ruoli.forEach(gruppo=>
              {
                this.gruppiUtente.push({ value: gruppo.value, description: gruppo.description });
              });
          });
      ...
      this.gruppiAttivi.forEach(gruppo => {
        ...
        let idxMieiGruppi = this.gruppiUtente.findIndex(el => el.value == gruppo.value);
        if (idxMieiGruppi >= 0) return;
        let index = this.organizzazioni.findIndex(el => el.label == fromGroupToGroupType(gruppo.value.split('_')[0]));
        if (index < 0) {
          //aggiungo il tipo organizzazione
          this.organizzazioni.push({ label: fromGroupToGroupType(gruppo.value.split('_')[0]), items: [] });
          index = this.organizzazioni.findIndex(el => el.label == fromGroupToGroupType(gruppo.value.split('_')[0]));
        }
        let indexOrganizzazione = this.organizzazioni[index].items.findIndex(el => el.value == this.withoutRole(gruppo.value));
        if (indexOrganizzazione < 0) {//aggiungo l'organizzazione a meno del ruolo
          this.organizzazioni[index].items.push({ value: this.withoutRole(gruppo.value), label: gruppo.description });
        }
        //aggiungo in mappa dei ruoli ammessi per l'organizzazione
        if (!this.ruoliAttiviOrganizzazione.has(this.withoutRole(gruppo.value))) {
          this.ruoliAttiviOrganizzazione.set(this.withoutRole(gruppo.value),
            new Set<String>([this.role(gruppo.value)]));
        } else {
          this.ruoliAttiviOrganizzazione.get(this.withoutRole(gruppo.value))
            .add(this.role(gruppo.value));
        }
      });
      this.loaded = true;
  ...
  buildForm() {
    ...
    this.formRichiesta = this.fb.group({
      nome: [this.userLogged.firstName, Validators.required],
      cognome: [this.userLogged.lastName, Validators.required],
      email: [this.userLogged.emailAddress, [Validators.required, Validators.email]],
      telefono: [telefono, [Validators.required, Validators.pattern(CONST.PHONE_PATTERN)]],
      fax: [fax, Validators.pattern(CONST.PHONE_PATTERN)],
      capoNome: ['', Validators.required],
      capoCognome: ['', Validators.required],
      capoEmail: ['', [Validators.required, Validators.email]],
      capoTelefono: ['', [Validators.required, Validators.pattern(CONST.PHONE_PATTERN)]],
      organizzazione: [null, Validators.required],
      ruolo: [null, Validators.required],
      inQualitaDi: [null, Validators.required],
  ...
  sendRichiesta() {
    ...
    this.loadingService.emitLoading(true);
    this.http.post(CONST.WEB_RESOURCE_BASE_URL + "/abilitazioni/richiesta-abilitazione-istruttoria.pjson", this.formData)
      .subscribe(
  ...
  selezioneGruppoRuoloValida(control: FormGroup): ValidationErrors | null {
    ...
    let idxGruppo = this.gruppiAttivi.findIndex(el => el.value == codice_gruppo);
    if (idxGruppo < 0) {
