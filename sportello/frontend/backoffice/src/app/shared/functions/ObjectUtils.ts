import { Template, TemplateComunicazione } from './../../features/gestione-istanza-comunicazioni/model/corrispondenza.models';
import { BasicFile } from 'src/app/shared/models/allegati.model';
import { isNullOrUndefined } from 'util';
import { ConfigOption, TemplateDestinatarioDTO } from '../models/models';
import { HierarchicalOption, OptionNode, TipiEQualificazioni, UlterioriContestiPaesaggistici, Allegato, TemplateEmailDestinatariDto } from './../models/models';
...
{
    let f: BasicFile = {
        formatoFile: a.tipiContenuto ? a.tipiContenuto[0] : "",
        nome: a.nome,
...
export function toTemplateComunicazione(template: TemplateEmailDestinatariDto[]): TemplateComunicazione[]
{
    let returningObj = [];
    for (let m of template)
    {
        ...
        returningObj.push(templateTemp);
    }
    return returningObj;   
}
