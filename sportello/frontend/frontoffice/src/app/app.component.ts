...
export class AppComponent implements OnInit
{
  ...
  public isLoadingMessageCustom: string =null;
...
sessionStorage.clear();
  ...
  */
  ...
  {
    this.loadingService.loadingObservable.subscribe((result:boolean)=>{
      if(result == false || this.isErrorMessageOpen == false){
        this.isLoadingOperation = result;
        if(!result)
          this.isLoadingDownload = result;
      }
      });
      this.loadingService.loadingDownloadObservable.subscribe((result:boolean)=>{
        if(result == false || this.isErrorMessageOpen == false){
          this.isLoadingDownload = result;
          if(!result)
            this.isLoadingOperation = result;
      }});
      this.loadingService.loadingObservableCustomMessage.subscribe((message:string)=>{
        this.isLoadingMessageCustom=message;
      });
  }
      ...
      (message: string) =>
      {
        ...
        if (this.isErrorMessageOpen == false)
        {
          switch (message)
          {
            ...
            case "PAGO_PA_ERROR": this.errorMessage = this.translate.instant("error.PAGO_PA_ERROR"         );break;
            case "AUTH_ERROR":
            ...
            default:
              if (message && message.startsWith("CUSTOM-OPERATION-TO-ADVICE:"))
