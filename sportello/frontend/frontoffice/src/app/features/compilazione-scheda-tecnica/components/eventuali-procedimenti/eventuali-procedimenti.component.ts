...
import { requiredNotEmpty } from 'src/app/shared/validators/customValidator';
...
export class EventualiProcedimentiComponent extends AbstractTab implements OnInit, OnChanges {
  ...
  ngOnInit() {
    this.buildForm();
    this.mappaOggettoForm();
    this.stFormService.eventualiProcedimentiFormComplete$.next(true);
  }
  ...
  get showDescrizione() {
    ...
    contenzisoAttoValue === true ?
      procedamentiDescrizione.setValidators([requiredNotEmpty]) :
