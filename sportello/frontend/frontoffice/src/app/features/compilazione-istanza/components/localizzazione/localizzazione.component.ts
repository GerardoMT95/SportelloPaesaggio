...
import { Allegato, ComuniCompetenza, LocalizazioneInfo, RequestAllegato } from 'src/app/shared/models/models';
...
export class LocalizzazioneComponent extends AbstractTab implements OnInit, OnDestroy {
  ...
  @Input("sezioniCatastali") sezioniCatastali: SelectOption[];
  ...
  */
  ...
  ngOnInit() {
    ...
    this.fascicolo.comuniCompetenza.sort((a,b)=>a.descrizione.localeCompare(b.descrizione)).forEach(el =>
      this.mockComuniSelezionabili.push({ value: el.enteId, description: el.descrizione, linked: el.codIstat,label:el.descrizione }));
    ...
    this.formInserimentoParticella = this.fb.group({
      ...
      particellaIntervento: [{ value: null, disabled: this.disable }, [Validators.required, Validators.maxLength(5)]],
      codiceEnte: ["", null],
      subIntervento: [{ value: "", disabled: this.disable }, [Validators.maxLength(5), Validators.pattern('[1-9]+[0-9]*')]],
      noteIntervento: [{ value: "", disabled: this.disable }]
    });
    this.loaded = true;
    this.istanzaFormService.localizzazioneFormComplete$.next(true);
  }
  ...
  buildForm() {
    ...
    this.mainForm.addControl('localizzazione', this.localizazioneForm);
  ...
  eliminaComune(index: number) {
    this.dialogService.showDialog(true,
      'LOCALIZZAZIONE.ALERT_ELIMINA_COMUNE',
      'ANNULA.TITLE',
  ...
  get getAllegatiShapeFiles() {
    return (this.localizazioneForm.get(
      'allegaShapeFile'
    ) as FormArray);
  }
...
*/
  /*get localizazioniInfo() {
    return this.localizazioneForm.get('localizazioniInfo');
  }*/
...
​​​​    //user_id: null
    ...
    if (oidsToDelete.size > 0 && this.tipoSelezioneLocalizzazione=='PTC') {
  ...
  /*elaboraParticelleCatturate(results: IdentifyResult[], comuneCompentenza: ComuniCompetenza) {
    ...
    for (let i in results) {
      ...
      nuova.sub = null;
      nuova.livello = compare.livello;
      //let particelleData=this.fascicolo.istanza.localizazione.localizzazioneComuni.find(el=>el.comuneId==comuneCompentenza.enteId).particelle;
  ...
  /*public popolaTabella(event: any): void {
    (this.mainForm.get("localizazione").get('allegaShapeFile').value).forEach((item, index) => {
      ((this.mainForm.get("localizazione").get('allegaShapeFile') as FormArray).at(index) as FormGroup).get('nome').patchValue(event.nomeFile);
      ((this.mainForm.get("localizazione").get('allegaShapeFile') as FormArray).at(index) as FormGroup).get('data').patchValue(new Date().toLocaleString());
      ((this.mainForm.get("localizazione").get('allegaShapeFile') as FormArray).at(index) as FormGroup).get('id').patchValue(event.idFile);
    });
  }*/

  private startWatchLocalizationsFormCheanges(): void {
    this.localizazioneForm.valueChanges.subscribe(values => {
      let value: LocalizzazioneValues = { comuneSelezionato: false, bpParchiRiserve: false, bpImmobileAreeInteresse: false };
  ...
  applyEditsCompleteCallback() {
    let f = (event) => {
      if (event.adds && event.adds.length > 0) {//sicuramente ha aggiunto una geometria!
        this.hasShape = true;
      } else {//forse ha tolto l'ennesima e dovrei interrogare il layer per capire se ce ne sono altre
    ...
    this.dichiarazioniService.deleteFile(requestAllegato).subscribe(
      response => {
        if (response.codiceEsito === "OK") {
          ((this.mainForm.get("localizazione").get('allegaShapeFile') as FormArray).at(0) as FormGroup).get('nome').patchValue("");
          ((this.mainForm.get("localizazione").get('allegaShapeFile') as FormArray).at(0) as FormGroup).get('data').patchValue(null);
          ((this.mainForm.get("localizazione").get('allegaShapeFile') as FormArray).at(0) as FormGroup).get('id').patchValue(null);
          this.loadingService.emitLoading(false);
        }
      },
      error => {
