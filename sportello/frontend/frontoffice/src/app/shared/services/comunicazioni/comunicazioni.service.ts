...
import { AllegatoCorrispondenza, CorrispondenzaSearch, DettaglioCorrispondenzaDTO, RespDettaglio, TemplateEmailDestinatariDto, TipologicaDTO } from './../../../features/interfaccia-di-riepilogo-del-fascicolo/models/corrispondenza.model';
...
{
  /*private URL_BASE   = CONST.WEB_RESOURCE_BASE_URL + "/comunicazioni";
  private URL_SEARCH = this.URL_BASE + "/search";
  private URL_INSERT = this.URL_BASE + "/create";
...
*/
  ...
  /**
   * @description Ricerca paginata corrispondenze
   * @param search 
   */
  /*public search(search: CorrispondenzaSearch): Observable<BaseResponse<PaginatedBaseResponse<DettaglioCorrispondenzaDTO>>>
  {
    let params = {params: toHttpParams(search)};
    return this.http.get<BaseResponse<PaginatedBaseResponse<DettaglioCorrispondenzaDTO>>>(this.URL_SEARCH, params);
  }
*/
  /**
   * @description Creo la bozza di una corrispondenza
   * @param idPratica 
   */
  /*public create(idPratica: string): Observable<BaseResponse<DettaglioCorrispondenzaDTO>>
  {
    let params = {params: new HttpParams().set("idPratica", idPratica)};
    return this.http.post<BaseResponse<DettaglioCorrispondenzaDTO>>(this.URL_INSERT, null, params);
  }*/

  /**
   * @description Esegue l'update della corrispondenza. Se bozza viene passato a "false"
   * la comunicazione viene inviata con questa chiamata. Il metodo verifica che la corrispondenza
   * che si tenta di aggiornare sia ancora in bozza prima di procedere
   * @param dettaglio 
   */
  /*public update(dettaglio: DettaglioCorrispondenzaDTO): Observable<BaseResponse<DettaglioCorrispondenzaDTO>>
  {
    return this.http.put<BaseResponse<DettaglioCorrispondenzaDTO>>(this.URL_UPDATE, dettaglio);
  }*/

  /**
   * @description Richiama la cancellazione della corrispondenza 
   * @param idPratica 
   * @param idCorrispondenza 
   */
  /*public delete(idPratica: string, idCorrispondenza: number): Observable<BaseResponse<boolean>>
  {
  ...
  /**
   * @description Eseguo l'upload del file per allegarlo alla corrispondenza
   * @param idPratica 
   * @param idCorrispondenza 
   * @param file 
   */
  /*public upload(idPratica: string, idCorrispondenza: number, file: File): Observable<BaseResponse<TipologicaDTO>>
  {
    let params = {params: new HttpParams().set("idPratica", idPratica).set("idCorrispondenza", idCorrispondenza.toString())};
    let formData = new FormData();
    formData.append("file", file);
    return this.http.post<BaseResponse<TipologicaDTO>>(this.URL_UPLOAD, formData, params);
  }*/

  /**
   * @description Rimuove un allegato dalla corrispondenza
   * @param idAllegato 
   * @param idPratica 
   */
  /*public remove(idAllegato: string, idPratica: string): Observable<BaseResponse<boolean>>
  {
    let params = { params: new HttpParams().set("idPratica", idPratica).set("idAllegato", idAllegato) };
  ...
  public getTemplatDestEmailList(sezione:string,idPratica:string): Observable<BaseResponse<Array<TemplateEmailDestinatariDto>>>
  {
    ...
    parameters = parameters.append("idPratica", idPratica);
    return this.httpClient.get<BaseResponse<Array<TemplateEmailDestinatariDto>>>(url,{ params: parameters });
  }
