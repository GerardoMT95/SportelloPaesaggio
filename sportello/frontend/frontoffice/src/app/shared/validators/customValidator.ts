...
import { isDataEmpty } from '../functions/ObjectUtils';
 ...
 * @param control 
 */
export function requiredNotEmpty(control: AbstractControl): ValidationErrors | null
{
    let validator: ValidationErrors | null = null;
    if (isDataEmpty(control.value))
        validator = { required: true };
    return validator;
}
...
/**
 * formControlNames nomi dei form control child da controllare
 */
export function requiredAllOrNone(formControlNames?:string[]): ValidatorFn
{
    return (control: AbstractControl): ValidationErrors | null =>
    {
        let response: ValidationErrors | null = null;
        let oks=0; //controlli pieni
        let numControls=0;
        let fcNames=formControlNames;
        if(!fcNames){
            fcNames=[];
           let fcKeys = (control as FormGroup).controls
           for (const name in fcKeys) {
            fcNames.push(name);
           }
        }
        fcNames.forEach(fcName=>{
                let val=control.get(fcName).value;
                if(!isDataEmpty(val)) {
                    oks++;
                  }
                numControls++;
            });
            //console.log('numControls:'+numControls+" oks:"+oks)
        if(numControls!=oks && oks!=0){
            //console.log('numControls:'+numControls+" oks:"+oks)
            response = { requiredAllOrNone: true };
        }
        return response;
    }
}
 ...
 */
export function dittaCf(): ValidatorFn
{
    return (control: AbstractControl): ValidationErrors | null =>
    {
        let response: ValidationErrors | null = null;
        let value=control.value as string;
        if(value && value.trim() && value.length==11){
            let reg=new RegExp('^[0-9]{11}$');
            if(!reg.test(value)){
                response = { pattern: true };
            }
        }else if (value && value.trim() && value.length==16){
            let reg=new RegExp(CONST.PATTERN_CF);
            if(!reg.test(value)){
                response = { pattern: true };
            }
        }else if(value && value.trim() && value.length>0){
            response = { pattern: true };
        }
        return response;
    }
...
export function requiredIfNotEmpty(dependency: AbstractControl): ValidatorFn
{
    return (control: AbstractControl): ValidationErrors | null =>
    {
        let response: ValidationErrors | null = null;
        if (dependency && dependency.value && control && (control.value=="" || !control.value)) 
        {
            response = { required: true };
        }
        return response;
