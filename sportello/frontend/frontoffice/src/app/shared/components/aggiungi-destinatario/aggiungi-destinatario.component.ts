...
import { DestinatarioComunicazioneDTO, DestinatarioSearch } from 'src/app/features/interfaccia-di-riepilogo-del-fascicolo/models/corrispondenza.model';
...
export class AggiungiDestinatarioComponent implements OnInit
{
  ...
  private buildFormInserimento(): void
  {
    this.formInserimento = this.fb.group({
      ...
      email: ['', [Validators.required, Validators.email,Validators.pattern(CONST.PATTERN_PEC)]],
      tipo: ['CC'],
      pec: [false],
      inRubrica: [false]
    });
  ...
  get rubricaEnteEnabled(): boolean { return false; }
