...
import it.eng.tz.puglia.aet.paesaggio.presentazioneistanza.servizi_esterni.webmail.dto.Risposta;
...
public class WebmailServiceImpl implements WebmailService {
	...
	public InvioMailResultDto inviaMail(InvioMailDto datiMail, Resource[] allegati, Long idCorrispondenza) throws Exception {
		...
		if (response.getStatusCode().is2xxSuccessful()) {
			...
			Integer resultUpdate = corrispondenzaRepository.updateMessageId(idCorrispondenza, result.getMailMessageId());
			if (resultUpdate > 0) {
	...
	/**
	 *Funzione per inviare al servizio di webmail la configurazione passata 
	 *
	 */
	@Override
	@Transactional(transactionManager=DatabaseConfiguration.TX_WRITE, propagation=Propagation.REQUIRED, readOnly=false, timeout=60000, rollbackFor=Exception.class)
	public Optional<ConfigurazioneCasellaPostaleResponseDto> configuraEAttivaCasellaPostale(ConfigurazioneCasellaPostaleDto ccpd) throws Exception {
		try {
			...
			if(esitoConfigurazione.getStatusCode().is2xxSuccessful()){
				...
				attivaCasellaPosta(ccprd);
				ConfigurazioneCasellaPostaleDTO ccpDTO = dk.creaDtoFromResponse(ccprd);
				Long resultUpdate;
				resultUpdate = (long) confCasPostService.insertOrUpdate(ccpDTO);
...
//	}
	...
	private void attivaCasellaPosta(ConfigurazioneCasellaPostaleResponseDto ccprd) throws Exception {
		//se è già attiva non la riattivo
		ResponseEntity<BaseResponse<ConfigurazioneCasellaPostaleDto>> response = webmailFeign.getConfigurazioneCasellaPostale(ccprd.getIndirizzoMail());
		if(response.getStatusCode().is2xxSuccessful() && response.hasBody() && response.getBody()!=null && response.getBody().getPayload()!=null && response.getBody().getPayload().getCasellaPostale().isCasellaAttiva()) {
	...
	private void disattivaCasellaPosta(String idCasellaPostale,String indirizzoCasella) throws Exception {
		AttivaDisattivaCasellaPostaleDto attivaDisattivaCasellaPostaleDto = new AttivaDisattivaCasellaPostaleDto();
		attivaDisattivaCasellaPostaleDto.setPk(idCasellaPostale);
		attivaDisattivaCasellaPostaleDto.setIndirizzoMail(indirizzoCasella);
