...
AS SELECT fascicolo.id,
  ...
  WHERE  (fascicolo.stato::text = 'TRANSMITTED'::text OR fascicolo.stato::text = 'SELECTED'::text OR fascicolo.stato::text = 'FINISHED'::text OR fascicolo.stato::text = 'ON_MODIFY'::text) AND fascicolo.esito::text <> 'NON_AUTORIZZATO'::text;
