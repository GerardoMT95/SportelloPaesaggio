import { DateOptions, ListInputsOptions } from './../model/form-search/formSearch.models';
import { FormBuilder, FormGroup, FormControl } from '@angular/forms';
import { Component, OnInit, Input, OnDestroy, Output, EventEmitter } from '@angular/core';
import { SearchFields, InputType } from '../model/form-search/formSearch.models';
import { Observable, Subscription } from 'rxjs';
import { swipe } from '../functions/genericFunctions';
...
export class FormSearchComponent implements OnInit, OnDestroy
{
  ...
  */
  ...
  public _organizedFormData: SearchFields[][] = [];
  ...
  ngOnInit()
  {
    ...
    {
      console.error("ATTENZIONE: parametro 'confguration' errato: ", errors);
      throw new Error();
    }
  }
  ...
  private validateConfiguration(): { formControlName: string, error: string }[]|null
  {
    ...
    if(ok)
    {
      ...
      this.configuration.forEach(field =>
      {
        ...
        if (withOptionInputs.includes(field.type) && field.extra &&
            (!(field.extra as ListInputsOptions).options || (field.extra as ListInputsOptions).options.length <= 0))
        {
  ...
  get dropdown(): InputType { return InputType.select; }
