...
import { BaseMap, basemap } from '../model/model';
import { BaseResponse } from '../model/base-response';
import { AllegatoCustomDTO, AllegatoDTO } from '../model/entity/allegato.models';
import { InfoAndGeoJson } from '../model/entity/localizzazione.models';
...
export class EsriMapComponent implements OnInit, OnDestroy {
  ...
  @Input() listaAllegati:AllegatoCustomDTO[]=[];
  ...
  @Output() aggiuntoShapeFile=new EventEmitter<any>();//serve a far refreshare la listaAllegati dal component padre

  @ViewChild('calcolaButton') public calcolaButton: ElementRef;
  @ViewChild('cancellaButton') public cancellaButton: ElementRef;
  @ViewChild('editorContainer') private editorContainer: ElementRef;
  @ViewChild('mapViewNode') private mapViewEl: ElementRef;
  @ViewChild('editorDiv') private editorDiv: ElementRef;
  ...
  public ngOnInit() {
    ...
    loadModules([
      ...
      var urlBackEnd=new URL(environment.webgis.urlLocale);
      ...
      async function generateFeatureCollection(fileName, files) {
        ...
        formData.append('idFascicolo', _self.idPiano);
        _self.loadingSvc.emitLoading(true);
        _self.hasLoaded = false;
        try {
          let response = await
            _self.httpClient.post<BaseResponse<InfoAndGeoJson>>(`${CONST.WEB_RESOURCE_BASE_URL}/geoutil/shape2geoJSON`, formData)
              .pipe(takeUntil(_self.unsubscribe$))
              .toPromise();
          _self.loadingSvc.emitLoading(false);
          if (response.payload && response.payload.fc && response.payload.fc.features.length>0) {
          ...
          if (response.payload.codeValidazione != 0) {
            ...
            _self.display = true;
            _self.typ = "success";
            _self.content = response.payload.messaggioValidazione || _self.translateService.instant('CORRECTLY_UPLOAD_SHAPE');
            _self.hasLoaded = true;
      ...
      function addGeoGsonToMap(fc) {
        ...
        fc.features.forEach((item) => {
          ...
          var geometry = new Polygon(
            ...
            */
          for(let idx = 0 ; idx < item.geometry.coordinates.length; idx++){
              var attributes = {};
              var symbol = new SimpleFillSymbol(SimpleFillSymbol.STYLE_SOLID, new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([250, 0, 33]), 2), new Color([253, 150, 44, 0.25]));
              attributes["FID"] = item.id + "_" + idx;
          //pull in any additional attributes if required
            attributes["COD_REG"] = "COD_" + item.id;
            attributes["REGIONE"] = "REG_" + item.id;
            attributes["SHAPE_Leng"] = item.id;
            ...
            attributes["is_custom"] =1;
            attributes["id_fascicolo"] = _self.idPiano;
              let geometry = new Polygon({ rings: item.geometry.coordinates[idx], "spatialReference": { "wkid": 32633 } });
              let graphic = new Graphic(geometry, symbol);
              graphic.setAttributes(attributes);
              arrayGraphics.push(graphic);
          ...
          */
          ...
          //_self.graphicLayer.add(graphic);
          for(let idx=0;idx<arrayGraphics.length;idx++){
            _self.graphicLayer.add(arrayGraphics[idx]);
          }
          
          _self.hasGraphic = true;//accende i bottoni di Conferma Area ed Elimina
        });//end loop features
        ...
        _self.map.addLayer(featureLayer);
        /*editLayerPolygon.applyEdits(features, null, null);
        editLayerPolygon.refresh();*/
      ...
      function addParticella(geometry, callback) {
        ...
        identifyTask.execute(identifyParams, function (results) {
          ...
          if(results.length>CONST.MAX_PARTICELLE){
            _self.display = true;
            _self.typ = "danger";
