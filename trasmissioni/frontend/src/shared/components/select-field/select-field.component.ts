...
@Component({
  ...
  template: `
    <div class="form-group">
      <label *ngIf="label" class="mr-3">{{ label }} <app-tooltip *ngIf="tooltip" [message]="tooltip"></app-tooltip></label>
      <select class="form-control" [id]="name" [name]="name" [value]="value" (onBlur)="onTouched()"
        ...
        <option *ngIf="!noPlaceholder" [value]="null" selected disabled hidden> {{placeholder && placeholder !=''?placeholder:('generic.selezionaunavoce'|translate)}}</option>
...
export class SelectFieldComponent extends BaseControlValueAccessor<string | number> 
{
  ...
  @Input("noPlaceholder") noPlaceholder: boolean = false;
